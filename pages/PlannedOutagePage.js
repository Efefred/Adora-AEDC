...
//   if (modalVisible) {
//     await expect(stayLoggedInButton).toBeVisible({ timeout: 10000 });

//     await stayLoggedInButton.click({ force: true });
...
//     await this.stayLoggedInIfPromptAppears();

//     await this.page.getByText('Outage & Availability', { exact: true }).click();
//     await this.page.getByText('Planned Outage Schedule', { exact: true }).click();

//     await expect(this.page).toHaveURL(/\/network-management\/planned-outage/i, {
//       timeout: 120000
//     });
...
//     await this.page.getByRole('button', { name: /create new outage/i }).click();

//     await expect(
//       this.page.getByText(/Is this Planned Outage Captured in the Load Forecast/i)
//     ).toBeVisible({ timeout: 60000 });

//     await this.page.getByRole('button', { name: /^yes$/i }).click();

//     await expect(this.page.getByText(/Select Asset Type/i)).toBeVisible({
//       timeout: 60000
//     });
//   }
...
//     await expect(radio).toBeVisible({ timeout: 60000 });
...
//       .locator('div')
//       .filter({ has: this.page.getByRole('heading', { name: new RegExp(headingText, 'i') }) })
//       .first();

//     const combo = section.getByRole('combobox').first();
...
//     await expect(textbox).toBeVisible({ timeout: 60000 });
//     await expect(textbox).toBeEnabled({ timeout: 60000 });
...
//     await expect(spinbutton).toBeEnabled({ timeout: 60000 });
...
//       .locator('.cdk-overlay-container, .mat-mdc-autocomplete-panel, .mat-autocomplete-panel, body')
//       .getByText(optionText, { exact: false })
//       .first();

//     await expect(option).toBeVisible({ timeout: 60000 });
...
//     await this.selectNativeDropdownByHeading('Select type of Outage', data.outageType);
//     await this.selectNativeDropdownByHeading('Select Outage Indication', data.outageIndication);

//     await this.fillTextboxByHeading('Reason for Outage', data.reason);

//     await this.selectNativeDropdownByHeading('Affected Business Unit', data.businessUnit);
//     await this.selectNativeDropdownByHeading('Undertaking Office', data.undertakingOffice);
//     await this.selectNativeDropdownByHeading('Affected DT', data.affectedDT);

//     await expect(
//       this.page.getByText(/Connected Feeder/i)
//     ).toBeVisible({ timeout: 60000 });

//     await this.selectAutocompleteByHeading('Permit Holder Name', data.permitHolderName);

//     await expect(
//       this.page.getByDisplayValue(data.permitHolderEmail).or(
//         this.page.getByText(data.permitHolderEmail)
//       )
//     ).toBeVisible({ timeout: 60000 });

//     await expect(
//       this.page.getByDisplayValue(data.designation).or(
...
//     await expect(
//       this.page.getByDisplayValue(data.expectedDuration).or(
//         this.page.getByText(data.expectedDuration)
//       )
//     ).toBeVisible({ timeout: 60000 });

//     await this.selectNativeDropdownByHeading('Weather', data.weather);
//     await this.fillNumberByHeading('Load Loss', data.loadLoss);

//     await expect(
//       this.page.getByRole('spinbutton').or(
//         this.page.getByText(/Number of Customers Affected/i)
//       )
//     ).toBeVisible({ timeout: 60000 });

//     await this.fillTextboxByHeading('Message to Affected Customers', data.message);
//   }
...
//     await expect(
//       this.page.getByText(data.pendingOutageMessage, { exact: false })
//     ).toBeVisible({ timeout: 60000 });
//   }
...
//     await expect(
//       this.page.getByText(/success|created|submitted/i)
//     ).toBeVisible({ timeout: 60000 });
//   }
...
//       .filter({ hasText: data.reason })
//       .filter({ hasText: data.affectedDT })
//       .filter({ hasText: data.permitHolderName })
//       .first();
//   }
...
//     await expect(row).toContainText(status, { timeout: 60000 });
//   }
...
//     const menuButton = row.locator('button, svg, .mat-icon, i').last();
...
//       .locator('.cdk-overlay-container, .dropdown-menu, body')
//       .getByText(actionName, { exact: true })
//       .first();

//     await expect(action).toBeVisible({ timeout: 60000 });
//     await action.click({ force: true });
//   }

//   async recommend(data) {
//     await this.chooseRowAction(data, 'Recommend');
//     await this.assertRecordStatus(data, 'Recommended');
//   }

//   async approveToFLApproved(data) {
//     await this.chooseRowAction(data, 'Approve');
//     await this.assertRecordStatus(data, 'FL Approved');
//   }

//   async approveToSLApprovedAndMove(data) {
//     await this.chooseRowAction(data, 'Approve');

//     await expect(this.getRecordRow(data)).toBeHidden({
//       timeout: 60000
//     });
...
//     await expect(row).toBeVisible({ timeout: 60000 });
//     await expect(row).toContainText(/SL Approved/i, {
//       timeout: 60000
//     });
//   }
...
//     await this.page
//       .getByText('HQ/Regional Control Center', { exact: true })
//       .click();

//     await expect(
//       this.page.getByText(/HQ\/Regional Control Center/i)
//     ).toBeVisible({
//       timeout: 60000
//     });
//   }
...
//     await this.chooseRowAction(data, 'Confirm');

//     await expect(this.page.getByText(/confirm/i)).toBeVisible({
//       timeout: 60000
//     });

//     const reasonBox = this.page.locator('textarea, input').last();

//     await reasonBox.fill('Outage is confirmed');

//     await this.page.getByRole('button', { name: /done/i }).click();

//     await expect(
//       this.page.getByText(/confirmed|success/i)
//     ).toBeVisible({
//       timeout: 60000
//     });
//   }
...
//     await this.page.getByText('Pending SG Card', { exact: true }).click();

//     await expect(
//       this.page.getByText(/Pending SG Card/i)
//     ).toBeVisible({
//       timeout: 60000
//     });
//   }
...
//     await this.chooseRowAction(data, 'Confirm SG Card');
...
// module.exports = { PlannedOutagePage };
